...
- **compact**: give the user the `/compact` line from the report's `Run:` block, verbatim in a code block. If the current step it names is out of date, correct that clause from the transcript rather than from memory. The user runs it. After compaction the SessionStart hook re-injects the pins automatically.
- **handoff**: the report's `Ask Claude:` block is the brief for the document; follow it. Write a handoff document for a fresh session. If a `handoff` skill is installed, invoke it; otherwise write the document yourself: goal, current state, decisions made and why, pinned constraints verbatim, files touched, open questions, next step. Save it outside the repo (scratch or temp directory), redact secrets, and tell the user to `/clear` and paste it.
- **abandon**: do not summarize from memory. The session's own account of itself is the thing in doubt. Instead list only durable artifacts the next session can read directly: `git status` and `git diff --stat`, recent commits, the issue or plan URL, and the pinned constraints. Tell the user to `/clear`, restate the task themselves, and point the new session at those artifacts. Offer the restart prompt from the report verbatim as a starting point; it is built from the user's own words and the pins only.
